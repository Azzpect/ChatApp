...
export function onConnection(socket: Socket) {
    ...
    socket.on("disconnect", () => {onDisconnection(userId)})
...
async function onChatMessage(message: string, receiverId: string, userId: string) {
    if(onlineUsers.has(receiverId)){
        ...
        onlineUsers.get(receiverId)?.emit("chat-message-received", {message: message, createdAt: new Date().toLocaleString()});
    }
    else 
